  ...
  return (
    <div className="space-y-5 rounded-b-3xl border-x border-b border-club-border bg-white/90 p-4 shadow-xl shadow-club-greenDeep/5 backdrop-blur sm:p-6">
      <section className="rounded-3xl bg-gradient-to-br from-club-greenDeep via-[#146C52] to-club-teal p-5 text-white shadow-lg shadow-club-greenDeep/15">
        <p className="text-xs font-bold uppercase tracking-[0.22em] text-[#CFEFE5]">Padel Group</p>
        <h2 className="mt-2 text-3xl font-black leading-tight sm:text-4xl">{group.name}</h2>
        <p className="mt-2 text-sm font-medium text-[#E8F6EF]">{activePlayers.length} active player{activePlayers.length === 1 ? '' : 's'} in the pool.</p>
      </section>
...
function GroupAction({ icon, title, detail, onClick }) {
  return (
    <button type="button" onClick={onClick} className="rounded-3xl border border-[#DDE7DE] bg-white p-4 text-left shadow-sm transition hover:bg-[#F1F7F2]">
      <span className="grid h-12 w-12 place-items-center rounded-2xl bg-[#E8F6EF] text-[#146C52]">{icon}</span>
      <span className="mt-4 block text-xl font-black text-[#18211C]">{title}</span>
      <span className="mt-1 block text-sm font-semibold text-[#65736A]">{detail}</span>
    </button>
  );
}
