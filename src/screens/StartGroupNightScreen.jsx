  ...
  return (
    <div className="space-y-5 rounded-b-3xl border-x border-b border-club-border bg-white/90 p-4 shadow-xl shadow-club-greenDeep/5 backdrop-blur sm:p-6">
      <section className="rounded-3xl bg-gradient-to-br from-club-greenDeep via-[#146C52] to-club-teal p-5 text-white shadow-lg shadow-club-greenDeep/15">
        <p className="text-xs font-bold uppercase tracking-[0.22em] text-[#CFEFE5]">Start New Night</p>
        <h2 className="mt-2 text-3xl font-black leading-tight sm:text-4xl">{group?.name || 'Group Night'}</h2>
        <p className="mt-2 text-sm font-medium text-[#E8F6EF]">{participantNames.length} selected for this tournament.</p>
      </section>
      ...
      <section className="rounded-3xl border border-[#DDE7DE] bg-white p-4 shadow-sm">
        <h3 className="text-lg font-black text-[#18211C]">Group Players</h3>
        {players.length ? (
          ...
          <p className="mt-3 rounded-3xl border border-dashed border-[#BFD0C2] bg-[#F1F7F2] px-4 py-8 text-center text-sm font-bold text-[#65736A]">No active group players. Add guests or update the pool.</p>
        )}
      </section>

      <section className="rounded-3xl border border-[#DDE7DE] bg-white p-4 shadow-sm">
        <h3 className="text-lg font-black text-[#18211C]">Guests</h3>
        <div className="mt-3 flex flex-col gap-2 sm:flex-row">
          <input
            ...
            className="min-h-14 flex-1 rounded-2xl border border-[#DDE7DE] px-4 font-semibold outline-none focus:border-[#168A5B] focus:ring-4 focus:ring-[#E8F6EF]"
            placeholder="Guest name"
          />
          <button type="button" onClick={handleAddGuest} className="min-h-14 rounded-2xl bg-[#168A5B] px-6 font-black text-white">
            Add Guest
          </button>
        </div>
        {guests.length > 0 && (
          <div className="mt-3 flex flex-wrap gap-2">
            {guests.map((guest) => (
              <span key={guest.id} className="inline-flex max-w-full items-center gap-2 rounded-2xl border border-[#DDE7DE] bg-[#F7FAF5] py-2 pl-3 pr-1 text-sm font-black text-[#18211C]">
                {guest.name}
                <button type="button" onClick={() => removeGuest(guest.id)} className="grid h-8 w-8 place-items-center rounded-xl text-[#8A978E] hover:bg-red-50 hover:text-red-600">
      ...
      <div className="sticky bottom-3 z-10 rounded-3xl border border-white/70 bg-white/90 p-3 shadow-2xl shadow-[#163B2E]/15 backdrop-blur">
        <div className="grid gap-2 sm:grid-cols-[auto_1fr]">
          <button type="button" onClick={() => setScreen('groupHome')} className="min-h-14 rounded-2xl border border-[#DDE7DE] px-5 font-black text-[#65736A] hover:bg-[#F1F7F2]">
            Back
          </button>
          <button type="button" onClick={startNight} disabled={isSaving || !setupStatus.isValid} className="min-h-14 rounded-2xl bg-[#168A5B] px-4 text-lg font-black text-white disabled:cursor-not-allowed disabled:bg-[#DDE7DE] disabled:text-[#65736A]">
  ...
  return (
    <button
      ...
      className={`rounded-3xl border p-3 text-left transition ${
        selected ? 'border-[#168A5B] bg-[#E8F6EF] shadow-[0_0_0_3px_rgba(22,138,91,0.10)]' : 'border-[#DDE7DE] bg-[#F7FAF5] hover:bg-[#F1F7F2]'
      }`}
    >
      <span className="flex items-center gap-3">
        <span
          className={`grid h-7 w-7 shrink-0 place-items-center rounded-full border-2 transition ${
            selected ? 'border-[#168A5B] bg-[#168A5B] text-white' : 'border-[#BFD0C2] bg-white text-transparent'
          }`}
          aria-hidden="true"
        >
        ...
        <span className="min-w-0">
          <span className="block truncate font-black text-[#18211C]">{player.name}</span>
          <span className="mt-1 block text-sm font-bold text-[#65736A]">Level {player.level}</span>
        </span>
      </span>
    </button>
  );
...
function SummaryTile({ icon, label, value, detail }) {
  return (
    <div className="rounded-3xl border border-[#DDE7DE] bg-white p-4 shadow-sm">
      ...
      <p className="mt-1 truncate text-3xl font-black tabular-nums text-[#18211C]">{value}</p>
      <p className="mt-1 line-clamp-2 text-xs font-bold uppercase tracking-wide text-[#8A978E]">{detail}</p>
    </div>
  );
}
...
function FieldSelect({ id, label, value, disabled, onChange, options }) {
  return (
    <div className="rounded-3xl border border-[#DDE7DE] bg-white p-4 shadow-sm">
      ...
      <select id={id} value={value} onChange={onChange} disabled={disabled} className="min-h-14 w-full rounded-2xl border border-[#DDE7DE] bg-[#F7FAF5] px-4 text-base font-black text-[#18211C] outline-none focus:border-[#168A5B] focus:ring-4 focus:ring-[#E8F6EF] disabled:opacity-60">
        {options.map((option) => (
